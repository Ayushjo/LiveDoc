...
function SignupForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      const { error } = await signUp.email({ name, email, password, callbackURL: postAuthUrl });
      if (error) {
  ...
  if (emailSent) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center px-8 py-12 bg-background">
        <div className="w-full max-w-[360px] text-center space-y-5 animate-scale-in">
      ...
      <div className="w-full max-w-[360px]">
        ...
        <div className="mb-8">
          <h1 className="text-2xl font-bold tracking-tight">
            {invite ? 'Create an account to join' : 'Create your account'}
          </h1>
